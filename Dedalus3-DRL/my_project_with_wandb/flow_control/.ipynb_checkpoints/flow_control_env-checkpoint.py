...
class FlowControlEnv(gym.Env):
    """
    Custom Environment for flow control using Dedalus simulations and OpenAI Gym interface.
    """
    def __init__(self):
        ...
        # Episode statistics
        self.episode_steps = 0  # Initialize step counter
        self.episode_count = 0  # Initialize episode counter
        self.episode_start_time = None  # Initialize start time for episodes
    ...
    def init_dedalus_simulation(self):
        ...
        # Second basis for running to take into account the dealias
        ...
        logger.info("Dedalus simulation initialized")
    ...
    def compute_reward(self, observation, previous_delta_N):
        """
        Computes the reward based on the deviation from the base state and the control effort.
        """
        U = self.u['g'][0]
        V = self.u['g'][1]
        ...
        ##### During episode rewards
        
        # Exponential reward scaling to favor smaller deviations
        exp_base_reward = 3 * np.exp(-delta_N)
        
        # Linear reward scaling to favor smaller deviations
        linear_base_reward = 3 * np.clip(1-delta_N/ self.delta_N0, -1, 1)
        
        # Quadratic reward for improvement
        improvement_reward = np.clip((max(0, previous_delta_N - delta_N) ** 2), 0, 1)

        # Stability reward for maintaining a low deviation
        stability_reward = 1 if abs(delta_N < 1)  else 0

        
        # Penalty that increases over time for larger actions
        time_penalty_factor = 0 if self.episode_steps == 0 else 1-1/self.episode_steps 
        control_penalty_time = 25 * abs(self.alpha_final) * time_penalty_factor
        # Global control penalty
        control_penalty_global = 3 * abs(self.alpha_final)
        # Penalty for large deviations
        large_deviation_penalty = np.clip(delta_N / self.delta_N0, 0, 1)
        # Total control penalty
        control_penalty = control_penalty_time + control_penalty_global + large_deviation_penalty

        
        reward = linear_base_reward + exp_base_reward + improvement_reward + stability_reward - control_penalty

        
        ##### End episode rewards        
        done = self.check_done(observation)   
        if done:
            # Terminal bonus for achieving near-zero deviation at the end of the episode
            terminal_bonus_win = 100 if delta_N < 0.01 else 0
            # Terminal bonus for achieving non abusive deviation at the end of the episode
            terminal_bonus_good = 20 if delta_N < 1 else 0
            
            # Terminal malus for doing worst deviation at the end of the episode
            terminal_malus = -100 if delta_N/ self.delta_N0 > 1 else 0
            # Terminal malus from being far from the goal at the end of the episode
            terminal_malus = -20 if 1-delta_N/ self.delta_N0 < 0.3 else 0

            reward +=  terminal_bonus_win + terminal_bonus_good - terminal_malus

        return reward, delta_N
    ...
    def check_done(self, observation):
        """
        Checks if the simulation has reached the end time or if the simulation has diverged.
        """
        if self.solver.sim_time >= self.initial_time + self.max_simulation_time:
            self._logger.warning('Maximum simulation time reached')
            return True
        ...
        if max_velocity > divergence_threshold:
            self._logger.warning('Simulation diverged with max velocity: %f', max_velocity)
            return True
    ...
    def reset(self, seed=None, **kwargs):
        ...
        if self.episode_start_time is not None:
            # Calculate the elapsed time for the previous episode
            episode_end_time = time.time()
            elapsed_time = episode_end_time - self.episode_start_time
            self._logger.info(f"Episode {self.episode_count} duration: {elapsed_time:.2f} seconds\n")
        # Record the start time for the new episode
    ...
    def prepare_agent_action(self, action):
        ...
        self._logger.info(f"Action chosen by the agent: {self.alpha_final:.4f} at time {self.solver.sim_time - self.initial_time:.1f}")
    ...
    def step(self, action):
        ...
        reward, self.previous_delta_N = self.compute_reward(observation, getattr(self, 'previous_delta_N', self.delta_N0))
        done = self.check_done(observation)   
        
        # Count episodes
        if done:
            self.episode_count += 1  # Increment episode count
            self._logger.info(f"Episode {self.episode_count} finished after {self.episode_steps} steps")  # Print step and episode count
            self.episode_steps = 0  # Reset step count for the next episode
            
        return observation, reward, done, {}, {}

    def set_saveable_state(self, state):
        """
        Sets the environment state from a saved state dictionary.
        """
        # Set the solver state
        self.solver.sim_time = state['sim_time']
        self.solver.iteration = state['iteration']
        self.t_i = state['t_i']
        self.alpha_final = state['alpha_final']
        self.signe = state['signe']
    ...
    def get_saveable_state(self):
        """
        Returns a dictionary of saveable environment state components.
        """
        state = {
            'delta_t': self.delta_t,
            'sim_time': self.solver.sim_time,
